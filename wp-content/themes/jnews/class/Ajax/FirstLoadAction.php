...
Class FirstLoadAction
{
    ...
    public function top_bar_account()
    {
        if(!is_user_logged_in()) {
	        ...
	        $account_url = apply_filters('jnews_top_bar_account_url', esc_url( home_url('/' . $endpoint['account']['slug']) ) );
    ...
    public function mobile_account()
    {
        ...
        if(!is_user_logged_in()) {
            ...
            $current_user = wp_get_current_user();
	        $endpoint = \JNews\AccountPage::getInstance()->get_endpoint();
            $account_url = apply_filters('jnews_mobile_account_url', esc_url( home_url('/' . $endpoint['account']['slug']) ) );
