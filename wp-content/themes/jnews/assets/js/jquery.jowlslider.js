...
(function ($) {
  ...
  $.fn.jowlslider = function (options) {
    ...
    return $(this).each(function () {
      ...
      var slider = $(this).addClass('owl-carousel')
      var flag = false
      var thumbnail = $(slider).parent().find(options.thumbnail).addClass('owl-carousel')
      var grab = false

      options.hover = $(slider).data('hover-action')
      options.autoplay = $(slider).data('autoplay')
      options.delay = $(slider).data('delay')


      $(slider).on('initialized.owl.carousel', function () {
        slider.siblings('.jeg_slider_placeholder').remove()
        slider.parent().addClass('jeg_slider_wrapper_loaded')
      }).owlCarousel({
        items: 1,
      ...
      }).on('hover', '.owl-item', function (e) {
        e.preventDefault()
        if (true === options.hover) {
          if (!grab) {
            $(slider).trigger('to.owl.carousel', $(this).index(), options.slideSpeed, true)
          }
        }
